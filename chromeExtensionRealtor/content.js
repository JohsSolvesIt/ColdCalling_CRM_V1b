...
console.log('🚀 Realtor Data Extractor content script loading...');

// Ensure safe module loading with error handling
function safeModuleAccess(moduleName, fallback = null) {
  try {
    return window[moduleName] || fallback;
  } catch (error) {
...
class ReviewsExtractor {
  constructor() {
    const ExternalReviewsExtractor = safeModuleAccess('ReviewsExtractor');
    if (ExternalReviewsExtractor) {
      this.externalExtractor = new ExternalReviewsExtractor();
    } else {
      console.warn('ReviewsExtractor module not available, using fallback');
      this.externalExtractor = {
        async extractReviews() { return []; },
        async expandTestimonials() { return false; }
      };
    }
  }

  async extractReviews() {
    return await this.externalExtractor.extractReviews();
  }

  async expandTestimonials() {
    return await this.externalExtractor.expandTestimonials();
  }
...
class RealtorDataExtractor {
  constructor() {
    console.log('🚨 REALTOR DATA EXTRACTOR CONSTRUCTOR CALLED 🚨');
    ...
    safeLog.info('🚀 Realtor Data Extractor content script loaded');
    safeLog.debug('📍 Current URL:', window.location.href);
    safeLog.debug('📄 Page title:', document.title);
    ...
    if (window.location.href.includes('realtor.com') || window.location.href.includes('localhost')) {
      safeLog.info('✅ Valid Realtor.com page detected, initializing...');
      this.init();
    } else {
      safeLog.debug('⚠️ Not on a Realtor.com page, skipping initialization');
    }
  }
  ...
  checkModuleAvailability() {
    ...
    if (availableCount !== totalCount) {
      console.warn(`⚠️ Only ${availableCount}/${totalCount} modules loaded`);
    } else {
      console.log(`✅ All ${totalCount}/${totalCount} modules loaded successfully!`);
    }
  ...
  init() {
    console.log('🚀 INIT CALLED - CHECKING AUTO EXTRACTION');
    ...
    chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
      ...
      if (request.action === 'extractData') {
        try {
          ...
          try {
            ...
            console.warn('⚠️ Agent data extraction failed:', error);
          ...
          console.log('📦 Structured data for popup:', structuredData);
      ...
      if (request.action === 'updateProfilePicture') {
        try {
          console.log('🖼️ PROFILE PICTURE UPDATE REQUEST RECEIVED');
          console.log('📍 Current URL:', window.location.href);
          console.log('🎯 Requested resolution:', request.resolution);
          ...
          // Update the profile picture with the requested resolution
          const result = await this.updateProfilePictureResolution(request.resolution);
          
          console.log('📸 Profile picture update result:', result);
          console.log('📸 New URL being returned:', result.newUrl);
  ...
  async checkAutoExtraction() {
    try {
      console.log('🚨 CHECK AUTO EXTRACTION CALLED 🚨');
      console.log('Current URL:', window.location.href);
      console.log('URL search params:', window.location.search);
      ...
      if (this.autoExtractionStarted) {
        console.log('🚨 AUTO-EXTRACTION ALREADY STARTED, SKIPPING 🚨');
      ...
      console.log('🚨 AUTO EXTRACT PARAM:', autoExtractParam, '🚨');
      ...
      if (autoExtractParam === 'true') {
        ...
        this.showNotification('🚀 Auto-Extraction Started', 'Force scrape in progress...', 'info', 2000);
        ...
        try {
          ...
          try {
            ...
            if (dbService) {
              const dbResult = await this.saveToDatabaseWithFeedback(dbService, extractedData);
              console.log('✅ Database save result:', dbResult);
            } else {
              console.warn('⚠️ DatabaseService not available - skipping database save');
            }
          } catch (dbError) {
            console.error('❌ Database save failed:', dbError);
            this.showNotification('⚠️ Database Save Failed', 'Data extracted but not saved to database', 'warning', 3000);
          }
          ...
          this.showNotification('✅ Auto-Extraction Complete!', 'Data extracted and saved to database', 'success', 3000);
          ...
          this.showNotification('❌ Auto-Extraction Failed', error.message, 'error', 5000);
        }
      }
    } catch (error) {
  ...
  async extractAllDataWithTimeouts() {
    ...
    try {
      ...
      results.agent = await this.extractAgentData();
      console.log('✅ Agent data:', results.agent);
      ...
      console.log('🎉 EXTRACTION COMPLETE - ALL DATA EXTRACTED!');
      this.sendProgressMessage('completion', `✅ All data extracted successfully in ${Math.round(results.metadata.totalDuration/1000)}s`);
      
      // 🚨 DEBUG: Log final data structure before returning
      console.log('🔍 FINAL EXTRACTION RESULTS SUMMARY:');
      console.log('📊 Agent data type:', typeof results.agent, results.agent ? Object.keys(results.agent).length + ' fields' : 'null');
      console.log('📊 Contact data type:', typeof results.contact, results.contact ? Object.keys(results.contact).length + ' fields' : 'null');
      console.log('📊 Listings data type:', typeof results.listings, results.listings ? Object.keys(results.listings).length + ' categories' : 'null');
      console.log('📊 Reviews data type:', typeof results.reviews, results.reviews ? 'recommendations: ' + (results.reviews.recommendations?.length || 0) : 'null');
      
      // 🔄 TRANSFORM TO LEGACY FORMAT FOR CRM COMPATIBILITY
      console.log('🔄 Applying client-side data transformation to results...');
      if (window.ClientDataTransformer) {
        try {
          const transformer = new window.ClientDataTransformer();
          const transformedData = transformer.transformToLegacyFormat(results);
          
          // Replace the nested structure with flat legacy format
          Object.assign(results, transformedData);
          console.log('✅ Results transformed to legacy CRM format successfully');
        } catch (error) {
          console.error('❌ Client transformation failed, using original data:', error);
        }
      } else {
        console.warn('⚠️ ClientDataTransformer not available, using original data format');
      }
      ...
      this.sendProgressMessage('error', `❌ Extraction failed: ${error.message}`);
      return results;
    }
  }

  // NEW: Extract listings with images - ENHANCED WITH URL-BASED ADDRESS EXTRACTION
  extractListingsWithImages() {
    console.log('🏠 Looking for property listings with images...');
    
    // 🚨 CRITICAL FIX: Use the enhanced ListingsExtractor instead of old method
    if (window.ListingsExtractor) {
      try {
        const listingsExtractor = new window.ListingsExtractor();
        console.log('✅ Using enhanced ListingsExtractor for property extraction');
        
        // Call the enhanced extraction method synchronously
        const result = listingsExtractor.performListingsExtractionSync(20);
        console.log('🏠 Enhanced extraction result:', result);
        return result;
      } catch (error) {
        console.error('❌ Enhanced extraction failed:', error);
        console.log('⚠️ Falling back to basic extraction');
        return this.extractListingsBasic();
      }
    } else {
      console.warn('❌ ListingsExtractor not available, falling back to basic extraction');
      return this.extractListingsBasic();
    }
  }

  // FALLBACK: Basic extraction method (renamed from original)
  extractListingsBasic() {
    const listings = {
      active: [],
      sold: []
    };
    ...
    if (propertyElements.length === 0) {
      console.log('🔍 No property cards found, looking for any containers with images and prices...');
      ...
      console.log(`✅ Found ${propertyElements.length} containers with images and prices`);
    }
    ...
    propertyElements.slice(0, 20).forEach((element, index) => { // Limit to 20 properties
      try {
        ...
        console.warn(`⚠️ Failed to extract property ${index + 1}:`, error);
      }
    });

    console.log(`✅ Extracted ${listings.active.length} active and ${listings.sold.length} sold properties`);
    return listings;
  }
  ...
  extractSingleProperty(element, index) {
    ...
    if (property.images.length > 0 || property.price) {
      console.log(`✅ Extracted property ${index + 1}:`, property);
      return property;
    }
  ...
  async extractAllData() {
    try {
      ...
      const agentData = {
        url: window.location.href.split('?')[0], // Remove query parameters/slugs
      ...
      try {
        ...
        if (dbService) {
          ...
          log.info('🔄 Forcing database save (duplicate check bypassed for debugging)');
      ...
      // 🔄 TRANSFORM DATA TO LEGACY FORMAT IMMEDIATELY AFTER EXTRACTION
      console.log('🔄 Applying client-side data transformation...');
      let transformedData = agentData;
      
      if (window.ClientDataTransformer) {
        try {
          const transformer = new window.ClientDataTransformer();
          transformedData = transformer.transformToLegacyFormat({
          ...
          console.log('✅ Data transformed to legacy CRM format successfully');
        } catch (error) {
          console.error('❌ Client transformation failed, using original data:', error);
      ...
      return {
        success: true,
        data: transformedData, // Now using transformed legacy format
        propertyRows: this.createPropertyRows(transformedData), // Keep original for compatibility
        propertyColumns: this.createPropertyColumns(transformedData), // New column format
        database: dbResult, // Database operation result
        summary: {
          agent: transformedData.name || transformedData.agent?.name || 'Unknown',
          totalListings: (transformedData.properties?.length || 0),
          activeListings: transformedData.listings?.active?.length || 0,
          soldListings: transformedData.listings?.sold?.length || 0,
          reviewCount: transformedData.review_count || transformedData.reviews?.overall?.count || 0,
          rating: transformedData.ratings || transformedData.reviews?.overall?.rating || 0,
          images: {
            agent: transformedData.profile_image_url || transformedData.images?.agentPhoto ? 1 : 0,
            team: transformedData.images?.teamPhoto ? 1 : 0,
            gallery: transformedData.images?.galleryImages?.length || 0,
            property: transformedData.images?.propertyImages?.length || 0
          }
        }
      };
    } catch (error) {
  ...
  async showDuplicateNotification(existingAgent) {
    return new Promise((resolve) => {
      ...
      chrome.runtime.sendMessage({
        ...
        if (!response) {
          log.info(`🔄 Auto-updating duplicate agent: ${existingAgent.name} (batch mode)`);
  ...
  async saveToDatabaseWithFeedback(dbService, agentData) {
    try {
      ...
      const cleanAgentData = dbService.prepareAgentData({
        ...
        profile_image_url: agentData.agent?.photo || agentData.images?.agentPhoto,
      ...
      // 🖼️ DEBUG: Log image data before database save
      console.log('🖼️ DEBUG: Agent photo before save:', agentData.agent?.photo);
      console.log('🖼️ DEBUG: Sample property images:', allListings.slice(0, 2).map(prop => ({
      ...
      // 🖼️ DEBUG: Log cleaned property images
      console.log('🖼️ DEBUG: Sample cleaned property images:', cleanProperties.slice(0, 2).map(prop => ({
        address: prop.address,
        image_urls: prop.image_urls
      })));
  ...
  async extractAgentData() {
    ...
    log.debug('🔍 Checking AgentExtractor availability:', !!window.AgentExtractor);
    if (typeof window !== 'undefined' && window.AgentExtractor) {
      log.debug('✅ AgentExtractor found, using enhanced extraction');
      const agent = window.AgentExtractor.extractAgentData();

      
      // Add bio extraction - simplified approach
      try {
        agent.bio = this.extractSimpleBio();
      } catch (error) {
    ...
    log.error('❌ AgentExtractor module not available - falling back to basic extraction');
    log.debug('🔍 Available window objects:', Object.keys(window).filter(k => k.includes('Extract')));
    const agent = {};
    ...
    // Bio/Description - simple extraction
    try {
      agent.bio = this.extractSimpleBio();
    } catch (error) {
      console.error('❌ Error extracting bio:', error);
  ...
  extractAgentProfilePhoto() {
    ...
    log.error('❌ AgentExtractor module not available for profile photo extraction - using fallback');
  ...
  extractAreasServed() {
    ...
    log.error('❌ AgentExtractor module not available for areas served extraction');
    return '';
  }
  ...
  extractCleanAgentName() {
    ...
    log.error('❌ AgentExtractor module not available for agent name extraction - using fallback');
    
    // Fallback extraction - comprehensive name extraction
    const nameSelectors = [
    ...
    log.debug('🔍 Trying basic name selectors...');
    for (const selector of nameSelectors) {
      const element = document.querySelector(selector);
      if (element) {
        const text = element.textContent?.trim();
        log.debug(`📝 Found text in ${selector}: "${text}"`);
        if (text && text.length > 2 && text.length < 100 && !text.toLowerCase().includes('realtor')) {
          log.debug(`✅ Using name from ${selector}: "${text}"`);
          return text;
        }
      }
    }
  ...
  isValidName(text) {
    ...
    log.error('❌ AgentExtractor module not available for name validation');
    return false;
  }
  ...
  extractPriceRange() {
    ...
    log.error('❌ AgentExtractor module not available for price range extraction');
    const pageText = document.body.textContent;
    const priceMatch = pageText.match(/\$[\d,]+\s*-\s*\$[\d,]+[KM]?/);
    return priceMatch ? priceMatch[0] : null;
  }

  // Simple bio extraction - no complicated menu searching
  extractSimpleBio() {
    log.debug('🔍 Starting simple bio extraction...');
    
    // Try common bio selectors first
    const bioSelectors = [
      '[data-testid*="bio"]',
      '[data-testid*="about"]', 
      '.agent-bio',
    ...
    for (const selector of bioSelectors) {
      ...
      if (element) {
        ...
        if (text && text.length > 100) {
          log.debug(`✅ Found bio using selector: ${selector}`);
          return this.cleanSimpleBio(text);
        }
      }
    }
    
    // Fallback: look for paragraphs containing bio-like content
    const paragraphs = document.querySelectorAll('p, div');
    for (const p of paragraphs) {
      const text = p.textContent?.trim();
      if (text && text.length > 200 && this.looksLikeBioText(text)) {
        log.debug('✅ Found bio in paragraph content');
        return this.cleanSimpleBio(text);
      }
    }
    
    log.debug('❌ No bio found with simple extraction');
    return 'Not found';
  }

  // Simple bio text validation
  looksLikeBioText(text) {
    const lowerText = text.toLowerCase();
    const bioKeywords = ['real estate', 'agent', 'broker', 'experience', 'years', 'client', 'property'];
    const navigationWords = ['menu', 'navigation', 'footer', 'header', 'cookie', 'privacy'];
    
    const hasBioKeywords = bioKeywords.some(keyword => lowerText.includes(keyword));
    const hasNavigationWords = navigationWords.some(word => lowerText.includes(word));
    
    return hasBioKeywords && !hasNavigationWords;
  }

  // Simple bio cleaning
  cleanSimpleBio(bio) {
    if (!bio) return 'Not found';
  ...
  extractRating() {
    ...
    log.debug('❌ AgentExtractor disabled - using rating fallback');
    ...
    const pageText = document.body.textContent;
    const ratingMatch = pageText.match(/(\d+(?:\.\d+)?)\s*(?:star|★|⭐)/i);
    if (ratingMatch) {
  ...
  extractRecommendationsCount() {
    ...
    log.error('❌ AgentExtractor module not available for recommendations count extraction');
    const pageText = document.body.textContent;
  ...
  extractExperience() {
    ...
    log.debug('❌ AgentExtractor disabled - using experience fallback');
  ...
  extractOfficeData() {
    console.error('🔍 TESTING: extractOfficeData called');
    ...
    for (const pattern of officePatterns) {
      const match = pageText.match(pattern);
      if (match) {
        console.error('✅ FOUND OFFICE PATTERN:', match[0]);
    ...
    if (!office.name) {
      console.error('❌ NO OFFICE PATTERN FOUND');
      office.name = null;
    }
    
    console.error('🔍 FINAL OFFICE DATA:', office);
    return office;
  ...
  extractContactData() {
    log.debug('📞 Extracting contact information directly...');
    ...
    log.debug('📊 Direct extraction results:', contact);
    return contact;
  }

  // NEW: Simple office name extraction
  extractOfficeName() {
    log.debug('🏢 Looking for office name...');
    ...
    if (urlMatch) {
      ...
      if (company && company.length > 3) {
        ...
        log.debug(`✅ Found office from URL: ${officeName}`);
        return officeName;
      }
    }
    ...
    if (pageText.includes('Masiello')) {
      log.debug('✅ Found office name: Masiello');
      return 'Masiello';
    }
    ...
    for (const company of companies) {
      if (pageText.includes(company)) {
        log.debug(`✅ Found office name: ${company}`);
        return company;
      }
    }
  ...
  extractOfficeAddress() {
    log.debug('🏢 Looking for office address...');
    ...
    if (matches && matches.length > 0) {
      ...
      log.debug(`✅ Found office address: ${address}`);
      return address;
    }
  ...
  // NEW: Profile picture resolution updater
  async updateProfilePictureResolution(resolution) {
    console.log(`🖼️ Starting profile picture update to ${resolution}x${resolution}`);
    
    try {
      // Find ANY image with rdcpix.com or realtor.com in the src
      ...
      for (const img of images) {
        ...
        if (src && (src.includes('rdcpix.com') || src.includes('realtor.com')) && src.includes('w260_h260')) {
          profileImage = img;
          originalUrl = src;
          break;
        }
      ...
      if (!originalUrl) {
        throw new Error('No profile picture found with w260_h260 format');
      }
      
      console.log(`📸 FOUND ORIGINAL URL: ${originalUrl}`);
      
      // Simple replacement - just replace w260_h260 with the new resolution
      const newUrl = originalUrl.replace('w260_h260', `w${resolution}_h${resolution}`);
      ...
      if (profileImage) {
        ...
        if (profileImage.dataset.src) {
          profileImage.dataset.src = newUrl;
        }
      }
  ...
  async extractReviews() {
    log.debug('🔍 Delegating to ReviewsExtractor...');
    if (!this.reviewsExtractor) {
      this.reviewsExtractor = new ReviewsExtractor();
    }
    return await this.reviewsExtractor.extractReviews();
  }
  ...
  extractModernRealtorReviews(reviews) {
    ...
    if (reviews && reviews.recommendations && Array.isArray(reviews.recommendations)) {
      ...
      log.recommendation(`🎯 Total extracted: 0 recommendations (reviews object was invalid)`);
    }
  }
  ...
  */
  ...
  forceOpenDataWindow(data) {
    // Use the modular popup system for enhanced display
    safeLog.info('🚀 Opening enhanced data display...');
    
    // Debug: Log the data structure being passed to popup
    safeLog.debug('📊 Data structure being passed to popup:', {
      hasAgent: !!data.agent,
      agentFields: data.agent ? Object.keys(data.agent).length : 0,
      hasContact: !!data.contact,
      contactFields: data.contact ? Object.keys(data.contact).length : 0,
      hasListings: !!data.listings,
      activeListings: data.listings?.active?.length || 0,
      soldListings: data.listings?.sold?.length || 0,
      hasReviews: !!data.reviews,
      reviewType: Array.isArray(data.reviews) ? 'array' : typeof data.reviews,
      recommendations: data.recommendations?.length || data.reviews?.recommendations?.length || 0
    });
    
    // Initialize popup system if not already done
    if (!this.popupSystem) {
      this.popupSystem = new PopupSystem();
    }
    
    // Show data using modular popup system
    this.popupSystem.showDataInNewWindow(data);
  }
  ...
  showDataInNewWindow(data) {
    ...
    if (newTab) {
      ...
      log.info('📊 Extraction data opened in new tab');
    } else {
...
<body>
  ...
  generateEnhancedDataSections(data, summary) {
    ...
    html += `
      <div class="category-card">
        <div class="category-header">
          <div class="category-title">👤 Agent Information</div>
    ...
    html += `
      <div class="category-card">
        <div class="category-header">
          <div class="category-title">🏢 Office Information</div>
    ...
    html += `
      <div class="category-card">
        <div class="category-header">
          <div class="category-title">🏘️ Property Listings</div>
    ...
    // Also collect photos from individual listings
    const listingPhotos = [];
    const propertyListings = data.listings || {};
    if (propertyListings.active) {
      propertyListings.active.forEach(listing => {
        if (listing.photos && Array.isArray(listing.photos)) {
          listingPhotos.push(...listing.photos);
        } else if (listing.image) {
          listingPhotos.push(listing.image);
        }
      });
    }
    if (propertyListings.sold) {
      propertyListings.sold.forEach(listing => {
        if (listing.photos && Array.isArray(listing.photos)) {
          listingPhotos.push(...listing.photos);
        } else if (listing.image) {
          listingPhotos.push(listing.image);
    ...
    const allImages = [...propertyImages, ...allPropertyPhotos, ...listingPhotos];
    ...
    html += `
      <div class="category-card">
        <div class="category-header">
          <div class="category-title">⭐ Reviews & Recommendations</div>
      ...
      <div class="modal-overlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 10000; display: flex; align-items: center; justify-content: center;">
        <div class="modal-content" style="background: white; border-radius: 12px; width: 95%; max-width: 1000px; max-height: 90vh; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.15); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <div class="modal-header" style="background: linear-gradient(135deg, #2c5aa0 0%, #1a365d 100%); color: white; padding: 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #1a365d;">
            <div>
              <h2 style="margin: 0; font-size: 24px; font-weight: 600;">📊 CRM Data Extraction Results</h2>
          ...
          <div class="actions-bar" style="background: #f8fafc; padding: 15px 20px; border-bottom: 1px solid #e2e8f0; display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
            ...
            <button id="copy-json-btn" style="padding: 8px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: all 0.2s;">📋 Copy Data</button>
            <button id="download-json-btn" style="padding: 8px 16px; background: #10b981; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: all 0.2s;">💾 Download</button>
            <button id="open-new-window-btn" style="padding: 8px 16px; background: #f59e0b; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: all 0.2s;">🔗 Open in Tab</button>
          </div>
          ...
          ${properties.slice(0, 8).map((property, index) => `
            <div style="border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px; background: #f8fafc;">
              <div style="display: grid; grid-template-columns: auto 1fr auto; gap: 10px; align-items: start;">
                ${(property.photos && property.photos.length > 0) || property.image ? `
                  <div class="property-images" style="display: flex; flex-wrap: wrap; gap: 4px; max-width: 120px;">
                    ${(property.photos && property.photos.length > 0 ? property.photos : (property.image ? [property.image] : [])).slice(0, 3).map(img => `
  ...
  createReviewsInfoSection(data) {
    ...
    return `
      <div class="info-section" style="padding: 12px; border-bottom: 1px solid #e2e8f0;">
        <h3 style="margin: 0 0 8px 0; color: #1e293b; font-size: 16px; font-weight: 600; display: flex; align-items: center; gap: 6px;">
          ⭐ Reviews & Testimonials <span style="background: ${reviews.length > 0 ? '#10b981' : '#ef4444'}; color: white; padding: 1px 6px; border-radius: 10px; font-size: 11px; font-weight: 500;">${reviews.length} found</span>
        </h3>
        ${reviews.length > 0 ? `
          <div class="reviews-grid" style="display: grid; gap: 8px;">
            ...
            ${reviews.length > 4 ? `
              <div style="text-align: center; padding: 6px; color: #64748b; font-style: italic; font-size: 11px;">
                ... and ${reviews.length - 4} more reviews (see raw data below)
              </div>
            ` : ''}
          </div>
        ` : `
      ...
      <div class="info-section" style="padding: 12px;">
        <details style="cursor: pointer;">
          <summary style="font-size: 16px; font-weight: 600; color: #1e293b; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
            📊 Raw Data (JSON) <span style="background: #6366f1; color: white; padding: 1px 6px; border-radius: 10px; font-size: 11px; font-weight: 500;">Click to expand</span>
          </summary>
          <pre style="background: #f1f5f9; padding: 10px; border-radius: 6px; overflow: auto; max-height: 300px; font-size: 10px; line-height: 1.3; border: 1px solid #e2e8f0; margin: 6px 0 0 0;">${JSON.stringify(data, null, 2)}</pre>
        </details>
      </div>
    `;
  }
...
// FORCE VISIBLE LOG - This should appear even if debug is off
console.log('🚨 REALTOR EXTENSION CONTENT SCRIPT LOADED 🚨');
console.log('Current URL:', window.location.href);
console.log('URL search params:', window.location.search);

safeLog.debug('🔄 Content script file executed, document.readyState:', document.readyState);
...
window.testExtraction = async function() {
  ...
  try {
    ...
    const result = await extractor.extractAllDataWithTimeouts();
...
window.testAutoExtraction = async function() {
  console.log('🧪 Testing auto-extraction trigger...');
  try {
    const extractor = new RealtorDataExtractor();
    await extractor.checkAutoExtraction();
    console.log('✅ Auto-extraction triggered');
  } catch (error) {
    console.error('❌ Auto-extraction failed:', error);
  }
