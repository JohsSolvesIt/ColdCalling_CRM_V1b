...
class RealtorDataExtractor {
  ...
  async extractAllData() {
    try {
      ...
      const agentData = {
        ...
        listings: this.safeExtract(() => this.extractListings()),
  ...
  expandAllSeeMoreContent() {
    ...
    // Return a promise that resolves after a short delay for DOM updates
    return new Promise(resolve => {
      setTimeout(() => {
        console.log(`🔍 DOM UPDATE DELAY COMPLETE - Ready for extraction`);
        resolve(expandedCount);
      }, expandedCount > 0 ? 1500 : 100); // Longer delay if we expanded content
    });
  }
  ...
  async extractCleanBio() {
    ...
    // Wait a moment for DOM updates after expansion
  ...
  isValidReviewContent(text) {
    ...
    // Additional checks for form-like content
    if (text.includes('EmailPhoneMessage') || 
        text.includes('Recaptcha') || 
        text.includes('Send email') ||
        text.includes('calls and texts') ||
        text.includes('autodialer') ||
        text.length > 500) { // Very long text is likely form content
      log.debug(`❌ Rejected form-like content: "${text.substring(0, 50)}..."`);
      return false;
    }
  ...
  looksLikeName(text) {
    ...
    return namePattern.test(text) && !excludeWords.includes(text);
  }
  ...
  extractListings() {
    ...
    // Wait longer for content changes after clicking tabs
    console.log('⏳ Waiting for listings content to load...');
    const waitTime = tabsClicked > 0 ? 2000 : 1000; // Wait longer if tabs were clicked
    const start = Date.now();
    while (Date.now() - start < waitTime) {
      // Pause for content loading
    }
  ...
  */
  ...
  // CRITICAL: Duplicate detection for listings
  isListingDuplicate(newListing, activeListings, soldListings) {
    ...
    return allListings.some(existingListing => {
      // Primary check: Same address
      if (newListing.address && existingListing.address) {
        const newAddr = newListing.address.toLowerCase().trim();
        const existingAddr = existingListing.address.toLowerCase().trim();
        if (newAddr === existingAddr) {
          return true; // Exact address match = duplicate
        }
      }
      
      // Secondary check: Same price + beds + baths (strong indicators)
      ...
      // Tertiary check: Same sqft + price (for properties without bed/bath info)
      if (newListing.price && existingListing.price &&
          newListing.sqft && existingListing.sqft) {
        
        if (newListing.price === existingListing.price &&
            newListing.sqft === existingListing.sqft) {
          return true; // Same price/sqft = likely duplicate
        }
      }
            ...
            ${recommendations.length > 0 ? `
                ...
                ${recommendations.slice(0, 3).map(rec => `
                    <div class="recommendation">
                        <div class="recommendation-text">"${rec.text?.substring(0, 200) || 'No text available'}${rec.text?.length > 200 ? '...' : ''}"</div>
                        <div class="recommendation-author">— ${rec.author || 'Anonymous'}</div>
                    </div>
                `).join('')}
                ${recommendations.length > 3 ? `<p style="text-align: center; color: #7f8c8d; font-style: italic;">... and ${recommendations.length - 3} more recommendations</p>` : ''}
                    ...
                    <div class="listing-card">
                        ...
                        <div class="property-details">
                            <div class="detail-item">
                                <div class="detail-value">${listing.beds || 'N/A'}</div>
                                <div class="detail-label">Beds</div>
                            </div>
                            <div class="detail-item">
                                <div class="detail-value">${listing.baths || 'N/A'}</div>
                                <div class="detail-label">Baths</div>
                            </div>
                            <div class="detail-item">
                                <div class="detail-value">${listing.sqft || 'N/A'}</div>
                                <div class="detail-label">Sq Ft</div>
                            </div>
                        </div>
                        <div><strong>Type:</strong> ${listing.propertyType || 'N/A'}</div>
            ...
            <div class="listings-grid">
                ${soldListings.slice(0, 6).map((listing, index) => `
                    <div class="listing-card">
                        ...
                        <div><strong>Type:</strong> ${listing.propertyType || 'N/A'}</div>
                        <div><strong>Status:</strong> ${listing.status || 'Sold'}</div>
                    </div>
                `).join('')}
            </div>
...
<body>
  ...
  cleanTextForCSV(text) {
    ...
    // Enhanced cleaning for review text and descriptions
    return text
      .replace(/\{[^}]*\}/g, '') // Remove CSS blocks
      .replace(/jsx-\d+/g, '') // Remove JSX class names
      .replace(/Overall rating \(\d+\)/gi, '') // Remove "Overall rating (5)" artifacts
      .replace(/ResponsivenessNegotiation skillsProfessionalism and communicationMarket expertise/gi, '') // Remove category labels
      .replace(/Responsiveness\s*Negotiation skills\s*Professionalism and communication\s*Market expertise/gi, '') // Remove spaced category labels
      .replace(/\b(Responsiveness|Negotiation skills|Professionalism and communication|Market expertise)\b/gi, '') // Remove individual categories
      ...
      .trim()
      .substring(0, 500); // Increase length limit for better review content
  }
