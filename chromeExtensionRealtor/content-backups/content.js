...
// Global availability check for modular components
console.log('🔧 Checking module availability at load time...');
if (typeof window.log === 'undefined') {
  console.error('❌ Logging utilities not loaded. Check module loading order.');
} else {
  console.log('✅ Logging utilities loaded');
}
if (typeof window.ContentPollingManager === 'undefined') {
  console.error('❌ ContentPollingManager not loaded. Check module loading order.');
} else {
  console.log('✅ ContentPollingManager loaded');
}
if (typeof window.DatabaseService === 'undefined') {
  console.error('❌ DatabaseService not loaded. Check module loading order.');
} else {
  console.log('✅ DatabaseService loaded');
}
if (typeof window.AgentExtractor === 'undefined') {
  console.error('❌ AgentExtractor not loaded. Check module loading order.');
} else {
  console.log('✅ AgentExtractor loaded');
}

// Use the global log object from logging-utils module
if (typeof window.log !== 'undefined') {
  window.log.info('🚀 Realtor Data Extractor content script loading...');
}
...
class RealtorDataExtractor {
  constructor() {
    ...
    this.bioExtractor = new (window.BioExtractor || class { async extractCleanBio() { return null; } })();
  ...
  checkModuleAvailability() {
    console.log('🔧 Checking module availability at runtime...');
    const modules = {
      'window.log': typeof window.log !== 'undefined',
      'window.ContentPollingManager': typeof window.ContentPollingManager !== 'undefined', 
      'window.DatabaseService': typeof window.DatabaseService !== 'undefined',
      'window.AgentExtractor': typeof window.AgentExtractor !== 'undefined',
      'window.BioExtractor': typeof window.BioExtractor !== 'undefined'
    };
    
    console.log('📊 Module availability status:', modules);
  ...
  async checkAutoExtraction() {
    try {
      console.log('🔍 Checking auto-extraction...');
      console.log('📍 Current URL:', window.location.href);
      console.log('🔧 URL Search Params:', window.location.search);
      
      // Prevent multiple auto-extractions
      if (this.autoExtractionStarted) {
        console.log('🔄 Auto-extraction already started, skipping duplicate request');
        return;
      }
      
      const urlParams = new URLSearchParams(window.location.search);
      const autoExtractParam = urlParams.get('autoExtract');
      console.log('🎯 autoExtract parameter:', autoExtractParam);
      
      if (autoExtractParam === 'true') {
        console.log('🚀 AUTO-EXTRACTION TRIGGERED by URL parameter');
        
        // Check module availability
        console.log('🔧 Module availability check:');
        console.log('  - log:', typeof window.log !== 'undefined');
        console.log('  - ContentPollingManager:', typeof window.ContentPollingManager !== 'undefined');
        console.log('  - DatabaseService:', typeof window.DatabaseService !== 'undefined');
        console.log('  - AgentExtractor:', typeof window.AgentExtractor !== 'undefined');
        ...
        console.log('🔄 Starting auto-extraction with intelligent timeout...');
        ...
        try {
          ...
          console.log('✅ Auto-extraction completed:', extractedData);
          ...
          this.showNotification('✅ Auto-Extraction Complete!', 'Data extracted and window opened', 'success', 3000);
  ...
  async waitForKeyContent() {
    console.log('⏱️ Intelligent content waiting started...');
    ...
    while (attempts < maxAttempts) {
      ...
      if (foundElements.length >= 2) { // At least 2 key elements found
        console.log(`✅ Key content ready after ${Date.now() - startTime}ms (${foundElements.length} elements found)`);
        return;
      }
    ...
    console.log(`⏰ Content wait timeout after ${Date.now() - startTime}ms`);
  }
  ...
  // Enhanced extraction with intelligent timeout management
  async extractAllDataWithTimeouts() {
    console.log('🎯 Starting enhanced data extraction with timeout management...');
    console.log('🔍 extractAllDataWithTimeouts() function called successfully');
    
    // Send progress message to frontend
    this.sendProgressMessage('initialization', 'Starting data extraction...');
    ...
    // Extract agent data first (usually fastest)
    console.log('👤 Extracting agent data...');
    this.sendProgressMessage('agent', 'Extracting agent information...');
    try {
      results.agent = await this.extractWithActivityTimeout(() => this.extractAgentData(), {
        name: 'agent',
        maxIdleTime: 10000, // 10 seconds idle timeout
        maxTotalTime: 30000, // 30 seconds total timeout
        activityCheckInterval: 500
      });
      console.log('✅ Agent data extracted successfully');
      this.sendProgressMessage('agent', 'Agent data extracted successfully');
    } catch (error) {
      console.error('❌ Agent extraction failed:', error);
      results.metadata.errors.agent = error.message;
      this.sendProgressMessage('agent', `Failed: ${error.message}`);
    }

    // Extract listings with property limit and smart timeout
    console.log('🏠 Extracting property listings (max 20 properties)...');
    this.sendProgressMessage('listings', 'Extracting property listings (max 20)...');
    try {
      results.listings = await this.extractWithActivityTimeout(() => this.extractListingsWithLimit(), {
        name: 'listings',
        maxIdleTime: 15000, // 15 seconds idle timeout
        maxTotalTime: 60000, // 60 seconds total timeout
        activityCheckInterval: 1000,
        propertyLimit: 20
      });
      console.log('✅ Listings extracted successfully');
      const totalProps = (results.listings?.active?.length || 0) + (results.listings?.sold?.length || 0);
      this.sendProgressMessage('listings', `${totalProps} properties extracted successfully`);
    } catch (error) {
      console.error('❌ Listings extraction failed:', error);
      results.metadata.errors.listings = error.message;
      this.sendProgressMessage('listings', `Failed: ${error.message}`);
    }

    // Extract recommendations (usually fast)
    console.log('⭐ Extracting recommendations...');
    this.sendProgressMessage('recommendations', 'Extracting recommendations and reviews...');
    try {
      results.reviews = await this.extractWithActivityTimeout(() => {
        // Use extractReviews() which returns a complete reviews object including recommendations
        // Wrap in Promise.resolve() to ensure it returns a Promise
        return Promise.resolve(this.extractReviews());
      }, {
        name: 'recommendations',
        maxIdleTime: 8000, // 8 seconds idle timeout
        maxTotalTime: 20000, // 20 seconds total timeout
        activityCheckInterval: 500
      });
      // Extract just the recommendations from the full reviews object
      results.recommendations = results.reviews?.recommendations || [];
      console.log('✅ Recommendations extracted successfully');
      this.sendProgressMessage('recommendations', 'Recommendations extracted successfully');
    } catch (error) {
      console.error('❌ Recommendations extraction failed:', error);
      results.metadata.errors.recommendations = error.message;
      this.sendProgressMessage('recommendations', `Failed: ${error.message}`);
    }
    ...
    console.log('🎯 Enhanced extraction completed:', results.metadata);
    this.sendProgressMessage('completion', `Extraction completed in ${Math.round(results.metadata.totalDuration/1000)}s`);
  ...
  sendProgressMessage(section, status, details = null) {
    ...
    console.log(`📊 Sending progress message:`, progressData);
  ...
  async extractWithActivityTimeout(extractionFn, options = {}) {
    ...
    console.log(`⏱️ Starting ${name} extraction with activity timeout (idle: ${maxIdleTime}ms, total: ${maxTotalTime}ms)`);
    ...
    const sendTimeoutMessage = (type, details) => {
      ...
      console.log(`⏰ Sending timeout message to parent:`, timeoutData);
    ...
    const timeoutCheck = setInterval(() => {
      ...
      // Reset activity flag
      if (isActive) {
        console.log(`🔄 ${name}: Activity detected, resetting idle timer`);
        isActive = false;
      }

      // Check for total timeout (hard limit)
      if (totalTime >= maxTotalTime && !timeoutTriggered) {
        console.log(`⏰ ${name}: Total timeout reached (${totalTime}ms)`);
      ...
      if (idleTime >= maxIdleTime && !timeoutTriggered) {
        console.log(`😴 ${name}: Idle timeout reached (${idleTime}ms with no activity)`);
        timeoutTriggered = true;
        clearInterval(timeoutCheck);
        activityMonitor.stop();
      ...
      if (propertyLimit && name === 'listings' && result) {
        ...
        if (totalProperties >= propertyLimit && !timeoutTriggered) {
          console.log(`🛑 ${name}: Property limit reached (${totalProperties}/${propertyLimit})`);
          timeoutTriggered = true;
          clearInterval(timeoutCheck);
          activityMonitor.stop();
  ...
  async saveToDatabaseWithFeedback(dbService, agentData) {
    try {
      ...
      console.log(`🏠 Properties being prepared for database: ${allListings.length}`);
      if (allListings.length > 0) {
        console.log(`🏠 Sample property:`, allListings[0]);
      }
      
      const cleanProperties = dbService.preparePropertyData(allListings);
      
      console.log(`🏠 Clean properties after preparation: ${cleanProperties.length}`);
      if (cleanProperties.length > 0) {
        console.log(`🏠 Sample clean property:`, cleanProperties[0]);
      }
  ...
  async extractAgentData() {
    console.log('🚀 extractAgentData() called - starting agent extraction...');
    
    // Check if AgentExtractor module is available
    if (typeof window !== 'undefined' && window.AgentExtractor) {
      console.log('✅ AgentExtractor module is available, calling extractAgentData()...');
      const agent = window.AgentExtractor.extractAgentData();
      console.log('📊 Agent data received from module:', agent);
  ...
  extractAgentProfilePhoto() {
    ...
    for (const selector of photoSelectors) {
      ...
      if (img && img.src) {
        console.log(`✅ Fallback found photo: ${img.src}`);
        return img.src;
      }
    }
    ...
    for (const selector of nameSelectors) {
      ...
      if (element) {
        ...
        if (text && text.length > 2 && text.length < 100) {
          console.log(`✅ Fallback found name: "${text}"`);
          return text;
        }
      }
    }
    ...
    if (title && title.includes('|') || title.includes('-')) {
      ...
      if (parts.length > 0) {
        ...
        if (potentialName.length > 2 && potentialName.length < 100) {
          console.log(`✅ Fallback found name in title: "${potentialName}"`);
  ...
  extractModernRealtorReviews(reviews) {
    ...
    if (reviews && reviews.recommendations && Array.isArray(reviews.recommendations)) {
      ...
      console.log('⚠️ reviews.recommendations is undefined or not an array, skipping cleanup');
      if (!reviews) reviews = {};
  ...
  extractListingsFromTextWithLimit(listings, remainingSlots) {
    ...
    console.log(`� Text-based extraction for up to ${remainingSlots} more properties...`);
      ...
      if (matches) {
        matches.forEach((match, index) => {
          ...
          if (listing) {
            ...
            console.log(`✅ Added text-based property: ${listing.price}`);
          }
        });
      }
    });
    
    console.log(`📝 Text extraction added ${extracted} properties`);
  }
  ...
  clickAllListingsTabSafely() {
    ...
    if (this._tabClickingInProgress || this._tabsAlreadyClicked) {
      console.log('🛑 TAB CLICKING: Already in progress or completed, skipping...');
      return 0;
    }
    ...
    for (let i = 0; i < combinedElements.length; i++) {
      ...
      if (clickedTabs.includes(text)) {
        console.log(`🔄 SKIP: Already clicked tab with text "${text}"`);
        continue;
      }
      ...
      if (isIndividualProperty) {
        if (i < 5) { // Only log first few to avoid spam
          console.log(`🏠 SKIP PROPERTY: "${text}" - This is an individual property, not a category tab`);
        }
        continue;
      }
      ...
      if (i < 10) { // Only log first 10 to prevent log spam
        ...
        console.log('🔍 TAB CHECK: ... (suppressing further logs to prevent spam)');
      }
      ...
      if (isAllTab || isWorkedWithBuyerTab || isActiveListingsTab || isSoldTab) {
        console.log(`🎯 FOUND CATEGORY TAB: "${text}" (testId: ${testId})`);
        ...
        for (let j = 0; j < clickStrategies.length && !clickSuccess; j++) {
          try {
            ...
            console.log(`✅ Successfully clicked tab: "${text}" (strategy ${j + 1})`);
            ...
            console.log(`⚠️ Click strategy ${j + 1} failed for "${text}":`, e);
          }
        }
  ...
  */
  ...
  extractAddress(element, text) {
    ...
    console.log('🔍 ADDRESS DEBUG: Full element text:', text.substring(0, 200));
    console.log('🔍 ADDRESS DEBUG: Element outerHTML preview:', element.outerHTML.substring(0, 500));
    ...
    if (linkWithAriaLabel) {
      ...
      if (ariaMatch) {
        ...
        if (address.match(/^\d{3,6}\s+[A-Z]/i) && address.length > 10 && address.length < 80) {
          console.log('✅ EXTRACTED FROM ARIA-LABEL:', address);
    ...
    if (communityMatch) {
      ...
      if (address.match(/^\d{3,6}\s+[A-Z]/i) && address.length > 10 && address.length < 80) {
        console.log('✅ EXTRACTED FROM COMMUNITY DETAIL:', address);
        return this.cleanAddress(address);
      }
    }
    ...
    for (const pattern of addressExtractionPatterns) {
      ...
      for (const match of matches) {
        ...
        if (!match.match(/square\s*feet?/i) &&
          ...
          console.log('✅ FOUND ADDRESS WITH PATTERN:', match);
    ...
    for (const nodeText of textNodes) {
      ...
      if (nodeText.match(/^\d{3,6}\s+[A-Z]/i) &&
        ...
        console.log('✅ FOUND ADDRESS IN TEXT NODE:', nodeText);
    ...
    for (const link of links) {
      ...
      if (urlMatch) {
        ...
        if (addressFromUrl.match(/^\d+\s+[A-Z]/i)) {
          console.log('✅ FOUND ADDRESS IN URL:', addressFromUrl);
          return this.cleanAddress(addressFromUrl);
        }
      }
    }
    
    console.log('❌ No valid address found - all extraction methods failed');
    return null;
    ...
    for (const pattern of streetPatterns) {
      ...
      if (streetMatches) {
        console.log('🔍 ADDRESS DEBUG: Street pattern matches:', streetMatches);
        for (const match of streetMatches) {
          ...
          if (!match.includes('$') && 
            ...
            console.log(`✅ STREET PATTERN FOUND: "${address}"`);
            return this.cleanAddress(address);
          }
        }
      }
    ...
    for (const selector of preciseAddressSelectors) {
      try {
        ...
        if (addressEl && addressEl.textContent.trim()) {
          ...
          console.log(`🔍 ADDRESS DEBUG: Selector "${selector}" found: "${addressText}"`);
          ...
          if (addressText.length > 10 && 
            ...
            console.log(`✅ SELECTOR ADDRESS FOUND: "${addressText}"`);
  ...
  forceOpenDataWindow(data) {
    ...
    // Handle both old format (data.data.agent) and new auto-extraction format (data.agent)
    let agent, contact, office, reviews, activeListings, soldListings, recommendations, summary, dbStatus;
    
    if (data.agent && data.listings && data.recommendations !== undefined) {
      // NEW AUTO-EXTRACTION FORMAT
      console.log('🔄 Using new auto-extraction format');
      agent = data.agent || {};
      contact = {}; // Will be extracted from agent data
      office = {}; // Will be extracted from agent data  
      reviews = { overall: {} }; // Will be extracted from recommendations
      activeListings = data.listings?.active || [];
      soldListings = data.listings?.sold || [];
      recommendations = data.recommendations || [];
      summary = { 
        totalActive: activeListings.length,
        totalSold: soldListings.length,
        totalReviews: recommendations.length,
        status: data.metadata?.errors ? 'FAILED' : 'SUCCESS'
      };
      dbStatus = false; // Auto-extraction doesn't save to DB yet
      
      // Extract contact info from agent data
      if (agent.phone) contact.primaryPhone = agent.phone;
      if (agent.email) contact.email = agent.email;
      if (agent.website) contact.website = agent.website;
      
      // Extract office info from agent data
      if (agent.officeName) office.name = agent.officeName;
      if (agent.officeAddress) office.address = agent.officeAddress;
      if (agent.officePhone) office.phone = agent.officePhone;
      
      // Extract reviews info from recommendations
      ...
      reviews = data?.data?.reviews?.overall || {};
      activeListings = data?.data?.listings?.active || [];
      soldListings = data?.data?.listings?.sold || [];
      recommendations = data?.data?.reviews?.recommendations || [];
