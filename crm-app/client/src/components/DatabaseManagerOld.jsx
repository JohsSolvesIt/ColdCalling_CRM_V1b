...
const DatabaseManager = ({ 
  ...
  const handleCreate = async () => {
    ...
    if (databases.includes(localNewDbName.trim())) {
  ...
  const handleRename = async (oldName, newName) => {
    if (!newName.trim()) {
      showToast('⚠️ Please enter a new database name', 'warning');
      return;
    }
    ...
    // Check for duplicates
    if (databases.includes(newName.trim())) {
      showToast('⚠️ Database with this name already exists', 'warning');
      return;
    }
    ...
    try {
      ...
      // If this was the current database, switch to the new name
      if (currentDatabase === oldName) {
        await switchToDatabase(newName.trim());
      }
      
      setEditingDatabase(null);
      setEditingName('');
      showToast(`✅ Database renamed from "${oldName}" to "${newName.trim()}"`, 'success');
    } catch (error) {
  ...
  const handleDelete = async (dbName) => {
    if (dbName === currentDatabase) {
      showToast('⚠️ Cannot delete the currently active database. Switch to another database first.', 'warning');
      return;
    }
  ...
  const switchToChromeExtension = async () => {
    try {
      setLoading(true);
      await api.switchDatabase('Realtor Database', 'chrome_extension');
  ...
  const rawCurrentDatabases = databases?.sqlite || [];
  const currentDatabases = rawCurrentDatabases.map(db => typeof db === 'string' ? db : (db.name || db));
  const chromeExtensionAvailable = databases?.chrome_extension?.length > 0;
          ...
          {activeTab === 'sqlite' ? (
            ...
            {databases.length === 0 && (
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-slate-600 dark:text-slate-400 mb-2">No Databases Found</h3>
                <p className="text-sm text-slate-500">
                  Create your first database to get started with the CRM
                </p>
              </div>
            )}
          </div>
          ...
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
            <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-md mx-4">
              ...
              <p className="text-sm mb-6">
                Are you sure you want to delete the database <strong>"{deletingDatabase}"</strong>? 
