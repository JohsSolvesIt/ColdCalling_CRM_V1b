    ...
    try {
      ...
      for (const contact of selectedContacts) {
        try {
          ...
          // Update the contact with new status and timestamp using patchRow
          const updateData = {
          ...
          const result = await patchRow(contact.id, updateData);
          
          console.log(`📥 patchRow Response:`, result);
      ...
      // Reload data to reflect changes
      console.log(`🔄 Reloading data to reflect status changes...`);
      await loadData();
